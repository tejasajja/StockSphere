...
Customization:
    ...
    python -m app.scripts.generate_data --agents 20 --customers 100 --transactions 300
...
async def insert_agents(collections, num_agents):
    for _ in range(num_agents):
        ...
        try:
            await collections["agents"].insert_one(agent.dict(by_alias=True))
            logging.info(f"Successfully inserted agent with ID: {agent.agent_id}")
...
def parse_args():
    ...
    parser.add_argument("--agents", type=int, default=10, help="Number of agents to generate (default: 10)")
    parser.add_argument("--customers", type=int, default=10, help="Number of customers to generate (default: 50)")
    parser.add_argument("--transactions", type=int, default=10, help="Number of transactions to generate (default: 200)")
